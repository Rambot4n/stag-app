...
export default function ProtectedLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      {/* Bottom nav */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 flex z-50">
        {NAV.map(({ href, label, icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex-1 flex flex-col items-center justify-center pt-1 pb-1.5 gap-0.5 transition-colors ${
